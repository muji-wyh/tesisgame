...
**Goal:** Make Word Buddies easier to enjoy repeatedly through forgiving practice,
...
existing model for practice and deck selection, and the existing UI for mode
choice and collection progress. Reuse the current audio, artwork, hints, rewards,
...
1. **Optional Practice mode:** Turn the existing mistake-counter area into a
   labeled Challenge/Practice toggle, without adding a row or reducing card
   sizes. Challenge remains the default and loses after three mistakes.
   Practice allows unlimited wrong attempts and still needs three real matches
   to earn one chest. Switching mode keeps the board, selection, hints, and
   matched pairs, but starts a fresh mistake allowance. Practice can resume a
   lost board; it cannot alter a won round or interrupt feedback. The chosen
   mode carries into replay for this session, not across browser reloads.
...
3. **Seasonal goals:** Show each season's collected count out of ten on its
   existing collection heading, with a clear completed state at ten. Remember
...
- Preserve the default three-mistake Challenge rule; Practice must be explicit.
...
- Save each earned reward once before animation; never award one for a mode switch.
...
| File | Responsibility |
| --- | --- |
| `scripts\game_model.gd` | Practice state, correct/wrong rules, and fresh deck selection. |
| `scripts\game_ui.gd` | The mode toggle, session season choice, and collection counts. |
| `scripts\word_card.gd` | Existing hint-star rendering, retained unchanged by the new mode. |
...
## Task 1: Forgiving practice

**Interface:** Add `practice_mode: bool = false` and
`set_practice(enabled: bool) -> bool` to the model. `reset()` retains the mode.

- [x] Add native assertions for five wrong attempts without a loss in Practice,
  three genuine pairs to win, a lost board resumed unchanged, and a fresh
  three-mistake allowance when returning to Challenge.
- [x] Assert that feedback and won/chest phases reject mode changes; check that
  switching preserves selection, hint IDs, and already matched pairs.
- [x] Run the native suite and observe those new assertions fail:

```powershell
node tools\run-godot.cjs --headless --path . --script res://tests/godot/run_tests.gd
```

- [x] Implement the mode transition in the existing model:

```gdscript
func set_practice(enabled: bool) -> bool:
    if phase in ["feedback", "won"] or (phase == "lost" and not enabled):
        return false
...
- [x] Increment `mistakes` only outside Practice, retain wrong feedback and
  streak reset in both modes, and guard the loss threshold with
  `not practice_mode`.
- [x] Wrap the existing mistake badges in `practice_button`; show the current
  mode underneath. In Practice replace the penalty badges with `No limit`.
  Disable the button during feedback and after a win. Ignore activation behind
  collection/preview modals. Use the existing `audio.interact`, `audio.cue`,
  `_refresh`, and accessibility announcement paths.
- [x] Add a real touch/controller browser flow that enters Practice, makes five
  errors, completes the board, and still gets exactly one reward. Cover resuming
  a loss and returning to Challenge without changing the cards.
- [x] Rerun the native suite and the relevant browser flows; retain the original
  default-Challenge loss assertions.
...
- [x] Add native assertions that manually choosing Summer survives unseeded
  replay, but does not override an explicit seed. Assert that mode and season
...
- [x] Render `Spring 3/10` for partial collections and
  `Spring complete! 10/10` for completion, using the actual season/count.
...
- [x] Update README and browser help to describe Practice, fresh boards, session
...
- [x] Run the existing Playwright runner for practice, fresh replay, season
...
- [x] Inspect desktop and small-phone screenshots in Challenge and Practice,
...
4. Compare production HTML's fingerprinted game pack to the built export;
  open the live game and exercise a hint, Practice, a win/chest, and collection
...
recover from a mistake, understand the selected mode, choose a season goal,
